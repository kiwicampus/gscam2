...
// subscribers to be 0
void wait_for_subscriber(std::shared_ptr<rclcpp::Node> node, const std::string& topic_name, bool to_be_available = true,
                         ...
                         std::chrono::microseconds sleep_period = std::chrono::seconds(1))
{
    ...
    printf("Waited %" PRId64 " microseconds for the subscriber to %s topic '%s'\n", time_slept_count,
           to_be_available ? "connect to" : "disconnect from", topic_name.c_str());
}
