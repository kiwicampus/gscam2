...
void GSCamNode::impl::process_frame()
{
...
#if defined(GSCAM2_USE_NVBUF)
  GstMemory * peek_mem = gst_buffer_peek_memory(buf, 0);
  if (peek_mem && gst_is_fd_memory(peek_mem)) {
    const int fd = gst_fd_memory_get_fd(peek_mem);
    if (fd >= 0) {
      NvBufSurface * surf = nullptr;
      if (NvBufSurfaceFromFd(fd, reinterpret_cast<void **>(&surf)) == 0 &&
          surf != nullptr && surf->numFilled > 0) {
        if (NvBufSurface2Raw(surf, 0, 0, width_, height_, img->data.data()) == 0) {
  ...
  if (buf_size < expected_frame_size) {
    RCLCPP_WARN(
      node_->get_logger(),
      "Image buffer underflow: expected frame to be %u bytes but got only %lu"
      " bytes (make sure frames are correctly encoded)", expected_frame_size, buf_size);
  }
  const gsize copy_len = (buf_size < expected_frame_size) ? buf_size :
    static_cast<gsize>(expected_frame_size);
  std::copy(buf_data, buf_data + copy_len, img->data.begin());
